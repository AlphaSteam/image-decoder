...
class GLibConan(ConanFile):
    ...
    def requirements(self):
        ...
        if is_apple_os(self) or self.settings.os == "Android":
            # Android needs external iconv, Apple prefers the system lib
    ...
    def generate(self):
        ...
        if self.settings.os == "Android":
            ...
            for incdir in iconv.cpp_info.includedirs:
                incpath = os.path.join(pkg, incdir)
                tc.c_args.append(f"-I{incpath}")
                tc.cpp_args.append(f"-I{incpath}")
            for libdir in iconv.cpp_info.libdirs:
                libpath = os.path.join(pkg, libdir)
                tc.c_link_args.append(f"-L{libpath}")
                tc.cpp_link_args.append(f"-L{libpath}")
                tc.c_link_args.append(f"-Wl,-rpath-link,{libpath}")
                tc.cpp_link_args.append(f"-Wl,-rpath-link,{libpath}")
