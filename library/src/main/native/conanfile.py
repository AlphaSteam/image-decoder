...
class VipsBuildConan(ConanFile):
    ...
    # Make libiconv an explicit, direct requirement so we can reference it here
    requires = (
        "glib/2.85.3",
        "pcre2/10.42",
        "libffi/3.4.4",
    ...
    def generate(self):
        # Provide pkg-config files for both host (Android) and build (macOS) contexts
        pc = PkgConfigDeps(self)
        pc.build_context_activated = [dep.ref.name for dep in self.dependencies.build.values()]
        pc.build_context_folder = "build-pc"
        pc.generate()
        VirtualBuildEnv(self).generate()

        # Compose Meson toolchain and inject iconv flags so GLib's dependency('iconv') probe succeeds
        ...
        # Libraries (iconv and, on Android, usually also charset)
